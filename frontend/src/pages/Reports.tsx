    ...
    <PageBody>
      ...
      <form onSubmit={handleSubmit}>
        ...
        <div>
          ...
          <input required id="year" type="number" name="year" className="border border-1" defaultValue={new Date().getFullYear()} />
        </div>
        <div>
          <label htmlFor="file">Upload a .csv file</label>
          <input required id="file" type="file" name="files" accept=".csv" className="border border-1" />
        </div>
        <p>Choose which reports to generate</p>
        <div>
          <label htmlFor="feesReport">Generate fees report</label>
          <input defaultChecked={true} type="checkbox" id="feesReport" name="feesReport" />
        </div>
        <div>
          <label htmlFor="pollinationSubsidies">Generate pollination subsidies report</label>
          <input defaultChecked={true} type="checkbox" id="pollinationSubsidies" name="pollinationSubsidies" />
        </div>
        <div>
          <label htmlFor="treatingSubsidies">Generate treating subsidies report</label>
          <input defaultChecked={true} type="checkbox" id="treatingSubsidies" name="treatingSubsidies" />
        </div>
